...
class pycontextSql(object):
    ...
    def populate(self,context,modifierFilters):
        ...
        for i in range(context.getNumberSentences()):
            ...
            self.__cursor.execute("""INSERT INTO sentences (num,txt) VALUES (?,?)""",(i,context.getText(),))
            ...
            for j in range(context.getNumMarkedTargets()):    
                ...
                if( not query ):
                    self.__cursor.execute("""INSERT INTO foundTargets (sentence,term,unmodified) VALUES (?,?,?)""",(sentid,term.getTerm(),'YES',))
                else:
