...
import pyConText.pyConTextGraph as pyConText
...
class criticalFinder(object):
    ...
    def __init__(self, dbname, outfile):
        ...
        self.query1 = '''SELECT reportid,impression FROM reports''' 
        ...
        # Quality targets
        self.targets = {"disease":critItems}
    ...
    def recordResults(self):
        query = """INSERT INTO results (id,disease, uncertainty, historical,
        quality,impression) VALUES (?,?,?,?,?,?)"""
        self.resultsCursor.execute(query,(self.currentCase,self.disease_state,self.uncertainty_state,self.historical_state,self.quality_state,self.currentText,))
...
def main():
    ...
    pec = criticalFinder(options.dbname, options.odbname)
    pec.processReports()
    pec.cleanUp()
